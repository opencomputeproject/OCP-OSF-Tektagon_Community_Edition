...
int cerberus_provisioning_root_key_action(struct pfr_manifest *manifest){
	...
	struct rsa_public_key root_key;
	...
	if(provision_header.provisioning_flag[0] == PROVISION_ROOT_KEY_FLAG){
		...
		pfr_spi_read(manifest->flash_id, manifest->address + CERBERUS_ROOT_KEY_LENGTH, sizeof(root_key)-1, &root_key);

		manifest->pfr_hash->start_address = manifest->address + CERBERUS_ROOT_KEY;
		manifest->pfr_hash->length = root_key.mod_length;
		...
		pfr_spi_write(ROT_INTERNAL_INTEL_STATE, CERBERUS_ROOT_KEY_ADDRESS, sizeof(root_key), &root_key);

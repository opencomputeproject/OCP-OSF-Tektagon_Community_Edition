...
static int setup(void)
{
	...
	ast_zassert_not_equal(dev, NULL,
			  "Device not found");
	...
	ast_zassert_equal(v1 & BIT(PIN_IN), 0,
	...
	ast_zassert_not_equal(v1 & BIT(PIN_IN), 0,
...
static int bits_physical(void)
{
	...
	ast_zassert_equal(raw_in(), false,
		      "raw clear mismatch");
...
static int pin_physical(void)
{
	...
	raw_out(true);
	ast_zassert_equal(gpio_pin_get_raw(dev, PIN_IN), raw_in(),
...
static int check_input_levels(void)
{
	...
	ast_zassert_equal(raw_in(), true,
		      "raw high mismatch");
	ast_zassert_equal(logic_in(), true,
		      "logic high mismatch");

	raw_out(false);
	ast_zassert_equal(raw_in(), false,
		      "raw low mismatch");
	ast_zassert_equal(logic_in(), false,
		      "logic low mismatch");
...
static int bits_logical(void)
{
	...
	ast_zassert_equal(logic_in(), !raw_in(),
...
int test_gpio_port(void)
{
	ast_zassert_equal(setup(), TC_PASS,
		      "device setup failed");
	ast_zassert_equal(bits_physical(), TC_PASS,
		      "bits_physical failed");
	ast_zassert_equal(pin_physical(), TC_PASS,
		      "pin_physical failed");
	ast_zassert_equal(check_raw_output_levels(), TC_PASS,
		      "check_raw_output_levels failed");
	ast_zassert_equal(check_logic_output_levels(), TC_PASS,
		      "check_logic_output_levels failed");
	ast_zassert_equal(check_input_levels(), TC_PASS,
		      "check_input_levels failed");
	ast_zassert_equal(bits_logical(), TC_PASS,
		      "bits_logical failed");
	ast_zassert_equal(check_pulls(), TC_PASS,
		      "check_pulls failed");
